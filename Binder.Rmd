...
title: "Data Management Plan for the Study - Do Reputable Open Access Journals Require
  Open Data Sharing"
author: "Principal Investigator - Nunya Exner, Virginia Commonwealth University"
...
According to the __Research Data Alliance__ (RDA) Data Policy Standardisation and Implementation group <https://www.rd-alliance.org/groups/data-policy-standardisation-and-implementation>:
...
``` {r plot_geog_of_journals, echo=TRUE, fig.width=10, fig.height=11}
...
Nunya Exner, Principal Investigator, `r params$institution`
...
- All original data, code, or reports produced as part of this project are owned by `r params$institution` per its institutional intellectual property policy.
    1. Code
    1. Data
    3. reports
    2. all IP
...
```{r include_equations, child="equations-child.rmd"}
...
A sample of the `doaj_seal.csv` data set is shown below.The complete dataset can be viewed in the [Annexes section](#annex-table)
...
|7| Website | hypertext | html file, 1MB | Publisher website (open access) and metadata |
...
ggplot(data=doaj, aes(doaj$JnlLicense, fill=doaj$JnlLicense)) + stat_count() + labs(x="License", y="Count", title = "DOAJ Seal by License") 


```
Figures generated using @R-tidyverse
...
This is auto-populated from the author's ORCID profile using the @R-rorcid package.
...
## Full dataset for `doaj_seal.csv`

```{r data-table}
doaj_seal %>%
  datatable(rownames = FALSE, 
            colnames = c("Title", "Publisher", "Country", "Fees", "Waivers", "Identifiers", "Start Year", "Language(s)", "Review  Process", "Plagiarism check", "Delay to Press", "License", "Author owns Copyright", "DOAJ Seal ARF ARF"),
